def deduplicate_sources(search_response: dict | list[dict]) -> list[dict]:
    """
    Takes either a single search response or list of responses from search API and de-duplicates them based on the URL.
...
def format_sources(
    ...
    """
    Takes a list of unique results from search API and formats them.
    Limits the raw_content to approximately max_tokens_per_source.
    include_raw_content specifies whether to include the raw_content from search in the formatted string.

    Args:
        sources_list: list of unique results from search API
        max_tokens_per_source: int, maximum number of tokens per each search result to include in the formatted string
        include_raw_content: bool, whether to include the raw_content from search in the formatted string
